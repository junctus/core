...
This is neo's **living internal security review**: the cumulative record of every adversarial pass over the cryptographic core and networked data plane, from the original two-round core analysis through the MPC-TLS / REALITY / circuit rounds and the full-codebase passes. It is an internal review and **not an external audit** — **neo remains unaudited and must not be relied on for real-world safety until the external cryptography-and-security audit gate** (`MILESTONES.md`). Every actionable finding below is remediated (each with a regression test where applicable) and the workspace is `fmt` / `clippy -D warnings` / `test` clean, but internal review only raises the floor; it is not the audit. Upstream-blocked and architectural residuals are called out explicitly rather than silently waived. The raw, un-deduplicated per-round logs live in git history.

## Cumulative findings

Every distinct finding from every round, deduplicated to its final resolution and sorted by round then severity. Round abbreviations: **R1/R2** = original core analysis (two rounds), **R3** = MPC-TLS / REALITY / circuit round, **R4** = full-codebase pass, **R5/R6** = full security/adversarial/privacy re-audits + remediation (see the dedicated sections below). Where the same issue was logged across rounds, the row carries its final status and cross-references the IDs.
...
## Round 5 (R5) — full security/adversarial/privacy re-audit + remediation

A fresh end-to-end pass over the whole workspace (seven parallel domain auditors — 2PC primitives, live MPC-TLS + committee integration, threshold/DKG crypto, core crypto/Sphinx, node data-plane/transport, discovery/seed/credits, and a cross-cutting privacy/traffic-analysis lens — plus a dependency-advisory, CI, deploy, and secret-scan sweep). The entire R1–R4 surface was re-probed and **held up** (the previously-remediated CRIT/HIGH fixes are all genuinely present, not cosmetic — see the section below). New findings clustered in the **committee 2PC-TLS exit**, the newest subsystem, which post-dates the R4 full-codebase pass. Every finding below is either **fixed** (with a regression test where it makes sense) or **documented as a scoped follow-on** with rationale.

| ID | Sev | Area | Finding (one line) | Status | Fix |
|----|-----|------|--------------------|--------|-----|
| R5-1 | HIGH | committee-2PC exit | Lead egress `StdTcpStream::connect(dest)` had **no SSRF guard, no port policy, no connect timeout** — an open proxy into loopback/RFC1918/cloud-metadata + internal port scanner, reachable on the live `--exit` relay (every *other* egress path was guarded) | fixed | `resolve_safe_dest` (resolve → reject any internal IP → dial the **exact vetted IP**, closing the DNS-rebind TOCTOU) + reduced-harm port policy + 10s timeout, threaded via `ExitPolicy` into `member_2pc_blocking`; dial now via new `netif::connect_scoped_blocking` (also fixes the interface-scoping LOW) |
| R5-2 | HIGH | discovery/snapshot | Anti-rollback `verify_fresh` had **zero production consumers** (the fetch path used plain `verify`), and its doc falsely claimed the client wired it — a CDN/mirror could freeze a client on an older, still-signed snapshot | fixed | `neo-ffi::fetch_relays` now calls `verify_fresh` with a process-lifetime high-water mark advanced on every accept; doc corrected (cross-restart persistence noted as a caller-supplied follow-on) |
| R5-3 | HIGH | 2PC-TLS claims | The **deployed** committee 2PC-TLS path runs the **semi-honest** networked engine (semi-honest garbling over unauthenticated ECtF MtA); "malicious-secure, live" overstated what ships | fixed (scoped) | Honest scope added to `committee_2pc` module doc + `CRYPTO.md`; the actionable code part (R5-5) fixed. Routing the live path through the authenticated engine is milestone work (see *Deferred*) |
| R5-4 | MED | committee-2PC | Follower **panics** on a `len==0` relay frame from a dishonest lead (`m[0]` index) — a targeted DoS on the follower's 2PC thread | fixed | `relay_recv` rejects empty frames (covers all four `m[0]` call sites) |
| R5-5 | MED | committee VSS seal | Sealed-partial keystream derived from the member's return secret **with no per-chunk nonce** → two-time pad across a multi-chunk response (a relay could recover the XOR of a member's partials) | fixed | chunk index bound into both the BLAKE3 keystream and the seal MAC; regression tests (no cross-chunk keystream cancellation; a partial can't open at another index) |
| R5-6 | MED | 2PC authgarble | In-process `EngineKind::Malicious` triple generation did **bucketing only, no sacrifice** → a corrupted-but-MAC-consistent triple (`αβ ≠ α∧β`) silently flips a gate | fixed | ported WRK17 `verify_triple` sacrifice to the authgarble share type; `bucketed_and_triples` now sacrifices every folded triple against a fresh independent one; regression test catches a biased product share |
| R5-7 | LOW | 2PC KOS | Statistical parameter `σ = 64` — a `2⁻⁶⁴` soundness/leakage margin, below the stack's 128-bit target | fixed | `SIGMA = 128` (`2⁻¹²⁸` margin); doc corrected |
| R5-8 | LOW | AKE/record | `PendingResponder.k_confirm` (and derived-key stack copies) not zeroized, despite R3-27 claiming so | fixed | `k_confirm` wrapped in `Zeroizing` (no `Drop` — keeps by-value moves legal under `forbid(unsafe_code)`) |
| R5-9 | LOW | seed | Per-key register cooldown stamped before `admit` → replaying a victim's public record (+ cheap PoW) burns their refresh quota | fixed | cooldown stamped only after `admit` accepts a new-or-newer record |
| R5-10 | LOW | discovery/DKG | DKG **commitment-equivocation** (a dealer sends different commitments to different peers → honest members split onto incompatible joint keys) named only in passing | fixed (doc) | boundary doc extended to name the vector + its fix (round-2 commitment-hash agreement); a liveness, not secrecy, gap — inside the documented "not Byzantine-tolerant" boundary |
| R5-11 | LOW | privacy | `neo committee2pc` demo CLI printed client IP + destination on one member's stdout (a client↔dest correlation) | fixed | client source address no longer logged |
| R5-12 | LOW | privacy | Client-side `debug!(%target …)` logged flow destinations | fixed | destination dropped from both circuit-open debug logs |
| R5-13 | LOW | committee-2PC | Rendezvous map could grow under a lead-payload flood | fixed | explicit `MAX_PENDING_RENDEZVOUS` cap |
| R5-14 | INFO | credits | `issue()` consumed an earned credit before validating the blinded element | fixed | validate the blinded element first |
| R5-15 | INFO | transport | QUIC `MAX_FRAME` 16 MiB vs the TCP path's 1 MiB | fixed | lowered to 1 MiB |
| R5-16 | INFO | live record | TLS record `seq` used unchecked `+= 1` (infeasible to reach, but inconsistent with R4-2) | fixed | `checked_add`, errors on exhaustion |
| R5-17 | INFO | committee-2PC | `Committee2pcPayload::encode` doc omitted the `lead_id` field | fixed | doc corrected |
| R5-18 | MED | netstack (client) | No caps on concurrent flows / half-open flows / UDP NAT table / per-flow `pending_write` (client-local self-DoS) | fixed | `MAX_FLOWS`/`MAX_UDP_FLOWS` caps + LRU eviction + `pending_write` bound + half-open idle reaping |
| R5-19 | INFO | supply chain | 5 RustSec advisories; the CI gate (`--deny warnings`) would be red, and the `sharks`/RUSTSEC-2024-0398 ignore was stale | superseded by R6-10 | libp2p upgraded in R6; all five vulnerable versions and the unsound `lru` release are gone |
| R5-20 | MED | committee-2PC / privacy | Both members' onion circuits reused **one shared path prefix** → the entry relay sat on *both* circuits and could correlate the client across the two committee halves (the linkage split-trust is meant to deny) | fixed | `committee_2pc_fetch` now takes a **separate path per member** and **enforces node-disjointness** across `{lead_path ∪ lead}` and `{follower_path ∪ follower}` (errors before any network I/O); both selectors (`neo-ffi::pick_committee`, desktop `run_committee_2pc`) pick a distinct path hop per member (min relays 3→**4**); regression tests for the guard + the selectors |

**Wire-format / deploy note.** Two fixes change on-the-wire crypto and require a **coordinated redeploy** of relays and clients (old ↔ new are not interoperable for those paths): **R5-5** (committee seal keystream/MAC derivation) and **R5-7** (KOS `σ`, i.e. the OT-extension row count). All other fixes are backward-compatible.

**Deferred — milestone-scale, documented not silently shipped:**
- **R5-3 core (route the live committee-2PC through the malicious-secure engine).** This is the project's own deferred performance milestone (constant-round networked authenticated online); rewriting the live path blind is riskier than the honest scoping now in place. The *toolkit* is fixed (R5-6) and correct.
- **H4 (uniform-size onion cells on the persistent-circuit path).** The circuit cell layer is plaintext-length-preserving, and cover cells are size-distinguishable, leaking flow structure to an on-path relay. A true fix is a **fixed-size cell layer** (chunking + reassembly across client/relays/exit) that also conflicts with the current "one cell = one UDP datagram (≤64 KiB)" design — a wire-format redesign of the **live** data plane that can't be validated against the running network in this pass. **Timing mixing + cover are already applied on the TUN-tunnel path** (`tunnel_stack.rs`/`tunnel.rs`, used by the shipping clients); the gap is cell-size uniformity on the raw circuit primitive. Tracked as its own milestone.
- **Committee-2PC follower-identity binding (LOW).** The lead accepts any token-bearing peer as the follower (the token is onion-secret, so exploitation already requires being a circuit endpoint or breaking the onion — no capability beyond a malicious member). Binding the follower's `NodeId` needs a third payload field + `peer_id` threaded through `serve_connection`→`handle_link`; deferred as disproportionate to a LOW.
- **Non-custody attestation replayability (INFO).** `NonCustodyProof` uses a prover-chosen challenge, so it attests "someone knew the key once," not current possession. Matches its documented *structural* scope; bind a verifier nonce/timestamp **if** it is ever used to gate participation.

## Round 6 (R6) — transport-linkability, resource, privacy, and supply-chain hardening

This pass reviewed every production entry path, committee rendezvous and blocking boundary, HTTP consumer, secret-file write, FFI process-global, deployment log, fuzz target, and locked dependency. It also re-ran the earlier adversarial checklist against the resulting call graph. The fixes are wire-compatible except for the operational trust change noted below.

| ID | Sev | Area | Finding (one line) | Status | Fix |
|----|-----|------|--------------------|--------|-----|
| R6-1 | HIGH | privacy / AKE | Clients reused a durable `NodeId` at every circuit entry, including both supposedly disjoint committee paths, allowing entry relays to link flows and committee halves | fixed | every client-to-entry connection now generates a one-use authenticated transport identity; relay-to-relay authentication remains stable; rotation regression test added |
| R6-2 | HIGH | committee-2PC | Unbounded bridge channels plus destination and whole-session I/O without deadlines let a malicious member or destination exhaust memory or pin workers indefinitely | fixed | bounded queues with backpressure, 30s destination read/write deadlines, 90s session budget, task cancellation, and bounded token-frame reads |
| R6-3 | HIGH | discovery trust | Shipped defaults were one mirror, one witness, threshold one: compromise of the sole operator could eclipse every zero-config client | fixed (fail-closed) | production default is threshold two; configuration errors until two trusted witnesses are provisioned; threshold one requires an explicit development override |
| R6-4 | MED | HTTP consumers | Snapshot, committee-list, and DoH consumers buffered response bodies without a streaming cap | fixed | content-length precheck plus streaming hard limits (34 MiB snapshots, 16 MiB committee lists, 256 KiB DoH); oversized-body regression test added |
| R6-5 | MED | initiator AKE | Outbound handshakes had no whole-handshake deadline, so a stalled peer could pin a client task | fixed | 10s initiator deadline covering connect and all handshake flights; stalled-peer regression test added |
| R6-6 | MED | privacy / logging | Caddy access logs retained client identifiers and the one-shot relay printed delivered plaintext payloads | fixed | discovery access logging disabled; relay logs only delivered byte counts; destination-bearing client debug logs remain removed by R5-12 |
| R6-7 | LOW | secret storage | Identity generation wrote then chmodded the final path and followed existing symlinks, creating disclosure/clobber races | fixed | same-directory `0600` create-new temporary, atomic no-clobber publication/replacement, symlink rejection, private `~/.neo`; mode/no-clobber/symlink tests added |
| R6-8 | LOW | FFI / routing | The process-global bound-interface setting was sticky and concurrent sessions could silently reroute one another | fixed | ref-counted interface leases reject conflicting indices and reset to unscoped on last close/drop; regression test added |
| R6-9 | LOW | fuzzing | The handshake fuzz target used a stale cookie API and did not synthesize the valid wrapper needed to reach deep parsing | fixed | target updated to current `CookieKey` flow and now exercises both arbitrary and valid-cookie inputs |
| R6-10 | MED | supply chain | CI permanently waived five reachable/locked vulnerabilities; all-feature resolution also contained an unsound `lru` | fixed / upstream-limited | libp2p 0.54→0.56 removes the five old vulnerable versions and `lru 0.12`; only two advisories in the locked-but-unreachable optional libp2p DNS backend remain narrowly ignored pending upstream hickory 0.26 support; `paste` remains an unmaintained, target-specific transitive warning |
| R6-11 | LOW | committee rendezvous | Reusing a live rendezvous token silently replaced its waiter, enabling targeted session disruption | fixed | duplicate tokens are rejected before map insertion |

**Operational trust note.** R6-3 intentionally makes the current one-witness baked client fail closed. Production zero-configuration discovery resumes only after a second independently operated witness signs snapshots and its key is shipped in the trusted bundle. `--threshold 1` is retained solely for local/development deployments.

**Residuals re-confirmed, not fixed by this pass:**
- **Deployed committee 2PC remains semi-honest.** R5-3 still applies: routing the live protocol through the malicious-secure engine is architectural milestone work, not a safe local patch.
- **Variable-size persistent circuit cells remain observable.** The R5 H4 boundary still requires a coordinated fixed-cell wire redesign.
- **Locked optional DNS advisories.** `hickory-proto 0.25.2` is not in any enabled neo feature graph, but Cargo locks it through libp2p's optional DNS dependency. RUSTSEC-2026-0118 has no fixed compatible release and 0119 requires hickory 0.26.1; CI exceptions are limited to these IDs and must be removed when libp2p upgrades.
- **Unmaintained `paste`.** It is target-specific and transitive through netlink crates; no vulnerability is reported, but it remains a maintenance risk until those upstreams migrate.
