...
Every distinct finding from every round, deduplicated to its final resolution and sorted by round then severity. Round abbreviations: **R1/R2** = original core analysis (two rounds), **R3** = MPC-TLS / REALITY / circuit round, **R4** = full-codebase pass, **R5** = full security/adversarial/privacy re-audit + remediation (see the dedicated section below). Where the same issue was logged across rounds, the row carries its final status and cross-references the IDs.
...
| R5-19 | INFO | supply chain | 5 RustSec advisories; the CI gate (`--deny warnings`) would be red, and the `sharks`/RUSTSEC-2024-0398 ignore was stale | fixed | advisory gate updated: stale ignore removed, the 5 transitive-via-`libp2p-tls`/`reqwest` advisories `--ignore`d with per-ID justification (the security-critical committee cert path uses the **patched** `rustls-webpki 0.103.13`), warnings surfaced non-gating so the gate stays meaningful for new vulns |
